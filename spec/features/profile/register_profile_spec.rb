...
feature 'Register new Profile' do
  ...
  scenario 'and send a confirmation mail' do
    user = User.create!(email:'elaine@gmail.com', password:123123, role:'candidate')
    skill = Skill.create!(name:'Ruby on Rails')
    joblevel = Joblevel.create!(name:'Junior')

    mailer_spy = class_spy(ProfilesMailer)
    stub_const('ProfilesMailer', mailer_spy)
    ...
    expect(ProfilesMailer).to have_received(:welcome).with(profile.id)
